...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export MANUFACTURER_CA_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/manufacturer.example.com/ca && ls *_sk)
    export LOGISTICS_CA_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/logistics.example.com/ca && ls *_sk)
    export WHOLESELLER_CA_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/wholeseller.example.com/ca && ls *_sk)
    export RETAILER_CA_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/retailer.example.com/ca && ls *_sk)
  fi
...
function networkDown() {
  ...
  IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE down --volumes --remove-orphans
...
function replacePrivateKey() {
  ...
  cd crypto-config/peerOrganizations/manufacturer.example.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
function generateChannelArtifacts() {
  ...
  set -x
  configtxgen -profile FourOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/WholesellerMSPanchors.tx -channelID $CHANNEL_NAME -asOrg WholesellerMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for WholesellerMSP..."
    exit 1
  fi
  echo
...
elif [ "$MODE" == "checkPrereqs" ]; then
  EXPMODE="checking the prerequisites"
else
  printHelp
  exit 1
fi
...
if [ "${MODE}" == "up" ]; then
  ...
  networkDown
  networkUp
elif [ "${MODE}" == "checkPrereqs" ]; then ## Upgrade the network from version 1.2.x to 1.3.x
  checkPrereqs
else
  printHelp
  exit 1
